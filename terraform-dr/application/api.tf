...
resource "aws_apigatewayv2_authorizer" "cognito" {
  ...
  name             = "BmazonDrCognitoJwtAuthorizer"
  authorizer_type  = "JWT"
  identity_sources = ["$request.header.Authorization"]
  jwt_configuration {
    audience = [aws_cognito_user_pool_client.web.id]
    issuer   = "https://cognito-idp.${var.dr_region}.amazonaws.com/${aws_cognito_user_pool.customers.id}"
  }
}
...
resource "aws_cloudwatch_metric_alarm" "order_errors" {
  alarm_name          = "bmazon-dr-order-errors"
  alarm_description   = "Temporary DR Order Lambda errors"
